...
# Convergence has been met for all optimizations!
...
for (i in 1:5) {
  for (j in 1:200) {
    res.temp <- read_rds(paste0("./", save.sims.dir, "/cv", i, "_sim", j, "_res.rds"))
    data.temp <- res.temp$pert.data
    n.temp <- data.temp$n
    n.neg.temp <- length(which(n.temp < 0))
    f.temp <- data.temp$f
    f.neg.temp <- length(which(f.temp < 0))
    m.temp <- data.temp$m
    m.neg.temp <- length(which(m.temp < 0))
    neg.temp <- sum(n.neg.temp, f.neg.temp, m.neg.temp)
    neg.check <- rbind(neg.check, c(i, j, neg.temp))
  }
}

# No negative data was generated! 