...
# of precision set by dt)
  ...
  while(p * dt <= max.t) {
    ...
    #No flux boundary condition, imaginary cells setting
...
calculate.sse <- function(par, grads, debug = FALSE) {
...
#Inputs:
# par - model parameter values, in a named vector
# gr - list of model gradients (see approximate.gradients outputs for elements)
...
# df - degrees of freedom for fit (number of data points - number of params)
  ...
  sd.ests <- sqrt(resid.var * 2 * diag(solve(optim.res$hessian)))
  return(sd.ests)
}
