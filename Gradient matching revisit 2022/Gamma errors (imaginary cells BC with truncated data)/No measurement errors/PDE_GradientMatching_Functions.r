...
generate.reference.data <- function(n.x11, max.x11, dt, max.t,
  dn, gamma, ita, dm, alpha, r, beta, n0, f0, m0, truncate = FALSE) {
...
# truncate - if true, trims off the first and last space point
...
# of precision set by dt)
  ...
  while(p * dt <= max.t) {
    ...
    #No flux boundary condition (imaginary cells)
    #n[1] <- n[2]
    #n[n.x11] <- n[n.x11 - 1]
  ...
  if(truncate){
    data.dim.row <- length(res.n[, 1])
    data.dim.col <- length(res.n[1, ])
    res.n <- res.n[2:(data.dim.row - 1), 2:(data.dim.col - 1)]
    res.f <- res.f[2:(data.dim.row - 1), 2:(data.dim.col - 1)]
    res.m <- res.m[2:(data.dim.row - 1), 2:(data.dim.col - 1)]
  }
